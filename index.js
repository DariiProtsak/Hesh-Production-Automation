...
for (let i = 0; i < chipCount; i++) {
    ...
    await chip.click({ timeout: 3000 }).catch(() => console.warn(`⚠ Не вдалося вибрати продукт ${i+1}`));
    ...
    if (hasParentContainers && childContainers.length > 0) {
        // Є і батьківські, і дочірні - обробляємо тільки дочірні
        ...
        // Є тільки дочірні без батьківських - не чіпаємо їх
        console.log(`⏭ Знайдено тільки дочірні без батьківських - пропускаємо`);
    } else if (hasParentContainers && childContainers.length === 0) {
        // Є тільки батьківські - не чіпаємо їх (так само як з дочірними без батьківських)
        console.log(`⏭ Знайдено тільки батьківські контейнери - пропускаємо`);
    } else {
...
// Чекаємо поки кнопка стане clickable (трохи очікуємо UI)
